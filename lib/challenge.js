...
prompt1()
// console.log(prompt1): this will print the function
...
// var can be access from directory level; follow the rules of "hoisting" of JS
...
function prompt11() {
  let median = nums.sort()[Math.floor(nums.length/2)];
  console.log(median);
}
prompt11()
...
function prompt12() {
  ...
  for (let i=1; i<=10; i++) {
    for (let j=11; j<=20; j++){
      ...
      myArray_2.push(j)
      console.log(`i: ${i} / j: ${j}`)
      console.log(`${myArray_1[i-1]} / ${myArray_2[j-11]}`)
    }
  }
}
prompt12()
// forEach can be attach to an arry; flatten the array; 2 pointer methods; .map; .filter/.reduce
// the const create dwill take space and the for loop will take time
